...
namespace MultivendorWebViewer.Manager
{
    public class OrderManager:SingletonBase<OrderManager>
    {
        ...
        public IEnumerable<Order> GetUserOrders(int userId)
        {
                ...
                using (var context = ServerModelDatabaseContextManager.Default.Context()/* ServerModelDatabaseContextManager.Default.Context()*/)
                {
            ...
            if (autoOrderNumberPropertyId.HasValue == false)
            {
                lock (autoOrderNumberLock)
                {
                    if (autoOrderNumberPropertyId.HasValue == false)
                    {
                        try
                        {
                            using (var ctx = ServerModelDatabaseContextManager.Default.Context())
                            {
